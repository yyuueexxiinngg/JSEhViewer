...
function getLatestVersion() {
  const current_version = JSON.parse($file.read(glv.appConfigPath).string).info
    .version;
  const url =
    "https://api.github.com/repos/Gandum2077/JSEhViewer/releases/latest";
  $http.get({
    url: url,
    timeout: 10,
...
async function generateTagTranslatorJson() {
  ...
  const info = resp.data;
  const dbUrl = info.assets.find(i => i.name === "db.text.json")
...
function getBilingualTaglist(taglist) {
  ...
  for (let tagType in taglist) {
    ...
    for (let tag of tags) {
      const trimedTag =
        tag.indexOf("|") !== -1 ? tag.slice(0, tag.indexOf("|")).trim() : tag;
...
module.exports = {
  ...
  translateTagType,
  getLatestVersion,
  generateTagTranslatorJson,
  updateTagTranslatorDict,
  translateTaglist,
  renderTaglistToText,
  getBilingualTaglist,
  getSearchUrl,
